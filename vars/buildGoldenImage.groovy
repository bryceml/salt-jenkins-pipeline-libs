def call(Map options) {
    ...
    def String vagrant_box_testing_versioned_url
                        ...
                        if ( macos_build == false ) {
                            ...
                            ansiColor('xterm') {
                                withCredentials([[$class: 'StringBinding', credentialsId: 'artifactory-apikey', variable: 'ARTIFACTORY_APIKEY']]) {
                                    withEnv([
                                        "PATH=/usr/local/sbin:/usr/local/bin:${current_path}",
                                        "ARTIFACTORY_URL=https://artifactory.saltstack.net/artifactory"
                                    ]) {
                                        sh """
                                        eval "\$(pyenv init -)"
                                        if [ ! -d /Users/parallels/.pyenv/versions/3.6.8 ]; then
                                            pyenv install 3.6.8 || echo "We already have this python."
                                        fi
                                        pyenv local 3.6.8
                                        pip freeze | grep -s invoke || pip install -r os-images/requirements/py3.6/base.txt
                            ...
                            withEnv([
                                ...
                                vagrant_box_provider = sh (
                                    script: """
                                    cat ${golden_images_branch}-manifest.json|jq -r ".builds[].custom_data.box_provier"
                                    """,
                                    returnStdout: true
                                    ).trim()
                                ...
                                vagrant_box_testing_versioned_url = sh (
                                    script: """
                                    cat ${golden_images_branch}-manifest.json|jq -r ".builds[].custom_data.box_testing_versioned_url"
                                    """,
                                    returnStdout: true
                                    ).trim()
                            }
    ...
    if ( macos_build == false ) {
        ...
        options['vagrant_box'] = "${vagrant_box_testing_versioned_url}"
    }
    ...
    try {
                        ...
                        stage('Promote AMI') {
                            try {
                                ...
                                node(jenkins_slave_label) {
                                    ...
                                    withAWS(credentials: 'os-imager-aws-creds', region: "${ec2_region}") {
                                        sh """
                                        pyenv install {{ pyenv3 }} || echo "We already have this python."
                                        pyenv local {{ pyenv3 }}
                                    ...
                                    withCredentials([[$class: 'StringBinding', credentialsId: 'artifactory-apikey', variable: 'ARTIFACTORY_APIKEY']]) {
                                        ...
                                        withEnv([
                                            ...
                                            jfrog rt move --url=\$ARTIFACTORY_URL --apikey=\$ARTIFACTORY_APIKEY --flat \
                                                --spec-vars='box_name=${vagrant_box_name};box_provider=${vagrant_box_provider};box_version=${vagrant_box_version};promoted=true'
        ...
        if ( macos_build == false ) {
            ...
            sh """
            vagrant box remove --force ${vagrant_box_testing_versioned_url} || true
            """
